...
def localWords(feed1,feed0):
	...
	for pairW in top30Words:
		...
		trainMat=[];trainClasses=[]
		for docIndex in trainingSet:
			trainMat.append(bagOfWords2VecMN(vocabList,dicList[docIndex]))
			trainClasses.append(classsList[docIndex])
		p0V,p1V,pSam=trainNB0(array(trainMat),array(trainClasses))
		errorCount=0
		for docIndex in testSet:
			wordVector=bagOfWords2VecMN(vocabList,docList[docIndex])
			if classifyNB(array(wordVector),p0V,p1V,pSam)!=classList[dicIndex]:
				errorCount+=1
			print ('the error rate is: ',float(errorCount)/len(testSet))
